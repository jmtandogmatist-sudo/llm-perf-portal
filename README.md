...
### 5. 极佳的 UI/UX 与配置管理 (Workspace & Observability)
* **IDE 式双栏工作流布局**：页面采用紧凑的双栏分割（控制栏 + 主工作区），消除滚动疲劳，将“配置与 YAML”、“监控终端”及“历史报告”深度整合成统一的工作区。
* **YAML 编辑双向实时绑定**：可视化表单与 YAML 编辑器左右对照并双向同步。修改任意一侧均能实时自动更新另一侧，并提供内联 YAML 语法错误纠错展示。
* **接口连通性前置校验 (Pre-flight Probe)**：支持一键测试 API 网关的可达性，提供可视化探针状态提示。
* **时序性能折线图集成 (Recharts Observability)**：压测完成后，直接在页面加载 Recharts 时序折线图，对平均延迟、首包延迟 (TTFT)、吞吐量 (TPS/QPS) 及被测主机的 GPU/显存/KV Cache 进行时序走势原地分析。
* **安全凭证加密与一键中止**：提供一键中止 (Abort) 压测的熔断保护；API Key 使用 `AES-256-CBC` 进行高强度加密存储。
